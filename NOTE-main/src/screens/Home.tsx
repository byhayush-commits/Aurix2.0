...
/**
 * Section data sources. Aurix's HomePage pulls these from a
 * getHomepageSections() cache backed by its own catalog; NOTE has no such
 * cache, so each section is a real, distinct search query instead of mock
 * content — same visual result, honest data underneath.
 */
...
/** Equalizer-style "now playing" badge — only on Continue Listening cards, per Aurix. */
...
/** One card component, sized per section — matches Aurix's per-size variants without duplicating five near-identical files. */
...
export default function HomeScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {/* Large title + single notification icon — no greeting, avatar or search bar, per Aurix's Home. */}
        <View style={[styles.header, { paddingTop: insets.top + SIZES.lg }]}>
          <Text style={styles.headerTitle}>Home</Text>
          <TouchableOpacity hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            <Bell color={COLORS.text.primary} size={23} />
          </TouchableOpacity>
        </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.hscroll}>
              {recentlyPlayed.slice(0, 10).map((track) => (
                <HomeCard
                  ...
                  onPress={playFrom(recentlyPlayed, 'Continue Listening')}
                />
              ))}
            </ScrollView>
          </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.hscroll}>
              {loading
                ? Array.from({ length: 5 }).map((_, i) => (
                    <SkeletonCard key={i} size={section.cardSize} radius={section.radius} />
                  ))
                : (sectionData[section.key] ?? []).map((track) => (
                    <HomeCard
                      ...
                      onPress={playFrom(sectionData[section.key] ?? [], section.title)}
                    />
                  ))}
            </ScrollView>
          </View>
